...
def test_roundtrip_is_stable():
    ...
    for raw in [
        ...
        once = main.serialize_notes(main.parse_notes(raw))
        twice = main.serialize_notes(main.parse_notes(once))
        assert once == twice, raw


def test_roundtrip_normalises_legacy_into_marked_form():
    assert main.serialize_notes(main.parse_notes("bare")) == "{} bare".format(MEMO)
