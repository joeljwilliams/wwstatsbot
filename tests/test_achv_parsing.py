...
def test_extracts_indented_rows_only():
    assert achv_handlers._extract_possible_achievements(REAL_MESSAGE) == [
        "Traffic Control",
        "Strongest Alpha",
        "Busy Night",
    ]
...
def test_dash_prefixed_player_name_is_not_read_as_an_achievement():
    ...
    names = achv_handlers._extract_possible_achievements(REAL_MESSAGE)
...
def test_dash_without_a_following_space_is_not_a_row():
    ...
    assert achv_handlers._extract_possible_achievements("-Someone\n - Real Achievement") == ["Real Achievement"]
...
def test_falls_back_to_unindented_rows_when_none_are_indented():
    ...
    assert achv_handlers._extract_possible_achievements("- One\n- Two") == ["One", "Two"]
...
def test_indented_rows_win_over_unindented_ones():
    ...
    text = "-PlayerName | tag\n - Real One\n- Not A Row"
    assert achv_handlers._extract_possible_achievements(text) == ["Real One"]


def test_multiple_dashes_and_tabs_are_accepted():
    assert achv_handlers._extract_possible_achievements("\t-- Double Dash") == ["Double Dash"]


def test_dedupes_case_insensitively_preserving_first_seen_order():
    text = " - Busy Night\n - busy night\n - Alpha\n - BUSY NIGHT"
    assert achv_handlers._extract_possible_achievements(text) == ["Busy Night", "Alpha"]


def test_trailing_whitespace_is_stripped():
    assert achv_handlers._extract_possible_achievements(" - Padded   ") == ["Padded"]


def test_empty_and_none_input():
    assert achv_handlers._extract_possible_achievements("") == []
    assert achv_handlers._extract_possible_achievements(None) == []


def test_no_rows_at_all():
    assert achv_handlers._extract_possible_achievements("Just a sentence.\nAnother line.") == []
...
def test_best_match_is_case_insensitive(achievements):
    assert achv_handlers._best_achievement_match("busy night")["name"] == "Busy Night"
    assert achv_handlers._best_achievement_match("BUSY NIGHT")["name"] == "Busy Night"


def test_best_match_returns_none_for_an_unknown_name(achievements):
    assert achv_handlers._best_achievement_match("no such achievement") is None


async def test_resolve_cards_reports_unmatched_names(achievements, no_fts):
    cards, not_found = await achv_handlers._resolve_achievement_cards(["Busy Night", "Nonsense XYZ"])
    assert len(cards) == 1
